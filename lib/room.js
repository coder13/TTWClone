...
Room.prototype.removeUser = function (user) {
	this.io.to(this.id).emit('userLeft', _.findWhere(this.users, {uuid: user}));
...
Room.prototype.addTime = function (user, time, index) {
	...
	this.io.to(this.id).emit('newTime', {user: user, time: time, index: index});
...
Room.prototype.update = function () {
	this.io.to(this.id).emit('syncRoom', {users: room.users, times: room.times});
};
